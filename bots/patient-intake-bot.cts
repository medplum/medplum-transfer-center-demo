...
type ValidLinkId = 'firstName' | 'lastName' | 'birthdate' | 'transferLocation';
type ParsedResults = { patient: Patient; nextAvailableRoom?: Location };
...
export async function handler(medplum: MedplumClient, event: BotEvent<QuestionnaireResponse>): Promise<void> {
  ...
  const parseItem = async (item: QuestionnaireResponseItem): Promise<void> => {
    const { patient } = results;

    switch (item.linkId as ValidLinkId) {
      case 'firstName': {
        const firstName = item.answer?.[0]?.valueString;
        if (!firstName) {
      ...
      case 'lastName': {
        const lastName = item.answer?.[0]?.valueString;
        if (!lastName) {
      ...
      case 'birthdate': {
        const birthDate = item.answer?.[0]?.valueDate;
        if (!birthDate) {
  ...
  for (const item of event.input?.item as QuestionnaireResponseItem[]) {
    await parseItem(item);
  }

  if (
    !(
      results.patient?.name?.[0]?.given?.length &&
      results.patient?.name?.[0].family &&
